...
class SSAH(object):
    ...
    def build_model(self):
        self.ph = {}
        self.ph['label_input'] = tf.placeholder(
            tf.float32, (None, 1, self.numClass, 1), name='label_input')
        self.ph['image_input'] = tf.placeholder(
            tf.float32, [None, self.image_size, self.image_size, 3], name='image_input')
        self.ph['text_input'] = tf.placeholder(
            tf.float32, [None, 1, self.dimText, 1], name='text_input')
        ...
        self.ph['lr_dis'] = tf.placeholder(
            'float32', (), name='lr_discriminator')
        self.ph['keep_prob'] = tf.placeholder('float32', (), name='keep_prob')
        self.ph['Sim'] = tf.placeholder(
            'float32', [self.num_train, self.batch_size], name='Sim')
        ...
        self.ph['L_batch'] = tf.placeholder(
            'float32', [None, self.numClass], name='L_batch')
        self.ph['B_batch'] = tf.placeholder(
            'float32', [None, self.bit], name='b_batch')
        self.ph['I_fea'] = tf.placeholder(
            'float32', [None, self.SEMANTIC_EMBED], name='I_fea')
        self.ph['T_fea'] = tf.placeholder(
            'float32', [None, self.SEMANTIC_EMBED], name='T_fea')
        self.ph['L_fea'] = tf.placeholder(
            'float32', [None, self.SEMANTIC_EMBED], name='L_fea')
        self.ph['L_fea_batch'] = tf.placeholder(
            'float32', [None, 1, self.SEMANTIC_EMBED, 1], name='L_fea_batch')
        self.ph['I_fea_batch'] = tf.placeholder(
            'float32', [None, 1, self.SEMANTIC_EMBED, 1], name='I_fea_batch')
        self.ph['T_fea_batch'] = tf.placeholder(
            'float32', [None, 1, self.SEMANTIC_EMBED, 1], name='T_fea_batch')

        # construct label network
        self.Hsh_L, self.Fea_L, self.Lab_L = self.lab_net(
            self.ph['label_input'], self.bit, self.dimLab)

        # construct image network
        self.Hsh_I, self.Fea_I, self.Lab_I = self.img_net(
            self.ph['image_input'], self.bit, self.numClass)

        # construct text network
        self.Hsh_T, self.Fea_T, self.Lab_T = self.txt_net(
            self.ph['text_input'], self.dimText, self.bit, self.numClass)

        # construct two discriminator networks
        self.isfrom_IL = self.dis_net_IL(
            self.ph['I_fea_batch'], self.ph['keep_prob'], reuse=False, name="disnet_IL")
        self.isfrom_L1 = self.dis_net_IL(
            self.ph['L_fea_batch'], self.ph['keep_prob'], reuse=True, name="disnet_IL")
        self.isfrom_TL = self.dis_net_TL(
            self.ph['T_fea_batch'], self.ph['keep_prob'], reuse=False, name="disnet_TL")
        self.isfrom_L2 = self.dis_net_TL(
            self.ph['L_fea_batch'], self.ph['keep_prob'], reuse=True, name="disnet_TL")

        # loss_D
        Loss_adver_IL = self.sce_loss(
            logits=self.isfrom_IL, labels=tf.zeros_like(self.isfrom_IL))
        Loss_adver_TL = self.sce_loss(
            logits=self.isfrom_TL, labels=tf.zeros_like(self.isfrom_TL))
        Loss_adver_L1 = self.sce_loss(
            logits=self.isfrom_L1, labels=tf.ones_like(self.isfrom_L1))
        Loss_adver_L2 = self.sce_loss(
            logits=self.isfrom_L2, labels=tf.ones_like(self.isfrom_L2))
        self.Loss_D = tf.div(Loss_adver_IL + Loss_adver_TL +
                             Loss_adver_L1 + Loss_adver_L2, 4.0)

        # train lab_net
        theta_L_1 = 1.0 / 2 * \
            tf.matmul(self.ph['L_fea'], tf.transpose(self.Fea_L))
        Loss_pair_Fea_L = self.mse_loss(tf.multiply(
            self.ph['Sim'], theta_L_1), tf.log(1.0 + tf.exp(theta_L_1)))
        theta_L_2 = 1.0 / 2 * tf.matmul(self.ph['H'], tf.transpose(self.Hsh_L))
        Loss_pair_Hsh_L = self.mse_loss(tf.multiply(
            self.ph['Sim'], theta_L_2), tf.log(1.0 + tf.exp(theta_L_2)))
        Loss_quant_L = self.mse_loss(self.ph['B_batch'], self.Hsh_L)
        Loss_label_L = self.mse_loss(self.ph['L_batch'], self.Lab_L)
        self.loss_l = alpha * Loss_pair_Fea_L + gamma * \
            Loss_pair_Hsh_L + beta * Loss_quant_L + eta * Loss_label_L

        # train img_net combined with lab_net
        theta_I_1 = 1.0 / 2 * \
            tf.matmul(self.ph['L_fea'], tf.transpose(self.Fea_I))
        Loss_pair_Fea_I = self.mse_loss(tf.multiply(
            self.ph['Sim'], theta_I_1), tf.log(1.0 + tf.exp(theta_I_1)))
        theta_I_2 = 1.0 / 2 * tf.matmul(self.ph['H'], tf.transpose(self.Hsh_I))
        Loss_pair_Hsh_I = self.mse_loss(tf.multiply(
            self.ph['Sim'], theta_I_2), tf.log(1.0 + tf.exp(theta_I_2)))
        Loss_quant_I = self.mse_loss(self.ph['B_batch'], self.Hsh_I)
        Loss_label_I = self.mse_loss(self.ph['L_batch'], self.Lab_I)
        Loss_adver_I = self.sce_loss(
            logits=self.isfrom_IL, labels=tf.ones_like(self.isfrom_IL))
        self.loss_i = alpha * Loss_pair_Fea_I + gamma * Loss_pair_Hsh_I + \
            beta * Loss_quant_I + eta * Loss_label_I + delta * Loss_adver_I

        # train txt_net combined with lab_net
        theta_T_1 = 1.0 / 2 * \
            tf.matmul(self.ph['L_fea'], tf.transpose(self.Fea_T))
        Loss_pair_Fea_T = self.mse_loss(tf.multiply(
            self.ph['Sim'], theta_T_1), tf.log(1.0 + tf.exp(theta_T_1)))
        theta_T_2 = 1.0 / 2 * tf.matmul(self.ph['H'], tf.transpose(self.Hsh_T))
        Loss_pair_Hsh_T = self.mse_loss(tf.multiply(
            self.ph['Sim'], theta_T_2), tf.log(1.0 + tf.exp(theta_T_2)))
        Loss_quant_T = self.mse_loss(self.ph['B_batch'], self.Hsh_T)
        Loss_label_T = self.mse_loss(self.ph['L_batch'], self.Lab_T)
        Loss_adver_T = self.sce_loss(
            logits=self.isfrom_TL, labels=tf.ones_like(self.isfrom_TL))
        self.loss_t = alpha * Loss_pair_Fea_T + gamma * Loss_pair_Hsh_T + \
            beta * Loss_quant_T + eta * Loss_label_T + delta * Loss_adver_T
        ...
        for epoch in range(Epoch):
            ...
            if epoch % 1 == 0:
                ...
                for idx in range(2):
                    ...
                    for train_labNet_k in range(k_lab_net//(idx + 1) + 1):
                        # Train lab_net
                        var['H'], var['LABEL_L'], var['feat_L'] = self.train_lab_net(
                            var, lr_lab)
                        var['B'] = np.sign(var['H'])
                        train_labNet_loss = self.calc_labnet_loss(
                            var['H'], var['LABEL_L'], var['feat_L'], Sim)
                        results['loss_labNet'].append(train_labNet_loss)
                        print(
                            '---------------------------------------------------------------')
                        print('...epoch: %3d, loss_labNet: %3.3f' %
                              (epoch, train_labNet_loss))
                        print(
                            '---------------------------------------------------------------')
            ...
            if epoch % 1 == 0:
                ...
                for idx in range(2):
                    ...
                    for train_disNet_k in range(k_dis_net):
                        ...
                        print('----------------------------------------')
                        print('..epoch:{0}, Loss_D:{1}, acc:{2}'.format(
                            epoch, Loss_D, acc))
                        print('----------------------------------------')
            ...
            for idx in range(3):
                ...
                for train_imgNet_k in range(k_img_net//(idx+1) + 1):
                    # Train img_net
                    var['F'], var['LABEL_I'], var['feat_I'] = self.train_img_net(
                        var, lr_img)
                    B_i = np.sign(var['F'])
                    if train_imgNet_k % 2 == 0:
                        ...
                        print(
                            '---------------------------------------------------------------')
                        print('...epoch: %3d, loss_imgNet: %3.3f' %
                              (epoch, train_imgNet_loss))
                        print(
                            '---------------------------------------------------------------')
            ...
            for idx in range(3):
                ...
                for train_txtNet_k in range(k_txt_net // (idx + 1) + 1):
                    var['G'], var['LABEL_T'], var['feat_T'] = self.train_txt_net(
                        var, lr_txt)
                    B_t = np.sign(var['G'])
                    if train_txtNet_k % 2 == 0:
                        train_txtNet_loss = self.calc_loss(
                            B_t, var['H'], var['G'], var['H'], Sim, var['LABEL_T'], train_L, alpha, beta, gamma, eta)
                        results['loss_txtNet'].append(train_txtNet_loss)
                        print(
                            '---------------------------------------------------------------')
                        print('...epoch: %3d, Loss_txtNet: %s' %
                              (epoch, train_txtNet_loss))
                        print(
                            '---------------------------------------------------------------')
    ...
    def test(self, phase):
        ...
        test['mapi2t'] = calc_map(
            test['qBX'], test['rBY'], self.query_L, self.retrieval_L)
        test['mapt2i'] = calc_map(
            test['qBY'], test['rBX'], self.query_L, self.retrieval_L)
        test['mapi2i'] = calc_map(
            test['qBX'], test['rBX'], self.query_L, self.retrieval_L)
        test['mapt2t'] = calc_map(
            test['qBY'], test['rBY'], self.query_L, self.retrieval_L)
        print('==================================================')
        ...
        savePath = '/'.join([os.getcwd(), 'Savecode',
                             dataset_bit_net + '.mat'])
    ...
    def train_lab_net(self, var, lr_lab):
        ...
        for iter in tqdm(range(num_train // batch_size + 1)):
            ...
            result = self.sess.run([self.Hsh_L, self.Lab_L, self.Fea_L], feed_dict={
    ...
    def generate_code(self, Modal, bit, generate):
        ...
        if generate == "label":
            ...
            for iter in tqdm(range(num_data // batch_size + 1)):
                ind = index[iter *
                            batch_size: min((iter + 1) * batch_size, num_data)]
                label = Modal[ind, :].astype(np.float32)
                label = label.reshape([label.shape[0], 1, label.shape[1], 1])
                Hsh_L = self.Hsh_L.eval(
                    feed_dict={self.ph['label_input']: label})
                B[ind, :] = Hsh_L
        elif generate == "image":
            num_data = len(Modal)
            index = np.linspace(0, num_data - 1, num_data).astype(int)
            ...
            for iter in tqdm(range(num_data // batch_size + 1)):
                ind = index[iter *
                            batch_size: min((iter + 1) * batch_size, num_data)]
                mean_pixel = np.repeat(
                    self.meanpix[:, :, :, np.newaxis], len(ind), axis=3)
                image = Modal[ind, :, :, :].astype(np.float64)
                image = image - \
                    mean_pixel.astype(np.float64).transpose(3, 0, 1, 2)
                Hsh_I = self.Hsh_I.eval(
            ...
            for iter in tqdm(range(num_data // batch_size + 1)):
                ...
                text = text.reshape([text.shape[0], 1, text.shape[1], 1])
                Hsh_T = self.Hsh_T.eval(
                    feed_dict={self.ph['text_input']: text})
                B[ind, :] = Hsh_T
        B = np.sign(B)
        return B
    ...
    def calc_labnet_loss(self, H, label_, feature, SIM):
        ...
        loss = alpha * term1 + gamma * term2 + beta * \
            term3  # + gama4 * term4 + gama5 * term5
        print('label:', term1)
    ...
    def calc_isfrom_acc(self, train_isfrom_, Train_ISFROM):
        erro = Train_ISFROM.shape[0] - np.sum(np.equal(
            np.sign(train_isfrom_ - 0.5), np.sign(Train_ISFROM - 0.5)).astype(int))
        acc = np.divide(np.sum(np.equal(np.sign(train_isfrom_ - 0.5),
                                        np.sign(Train_ISFROM - 0.5)).astype('float32')), Train_ISFROM.shape[0])
        return erro, acc
